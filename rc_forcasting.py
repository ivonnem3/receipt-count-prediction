...
# Generate Forcasting Model & Predictions
SARIMAXmodel = SARIMAX(reciept_data.Receipt_Count, order = (0,1,2), seasonal_order=(0,1,1,12))
SARIMAXmodel = SARIMAXmodel.fit()

start = len(train)
end = len(reciept_data)-1
predictions = SARIMAXmodel.predict(start=start,end = end, dynamic=True).rename('Predicted')

arma_rmse = np.sqrt(mean_squared_error(test["Receipt_Count"].values, predictions ))
